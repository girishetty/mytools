...
int Decompress(const TSRC* src, TDEST* dest, int srcSize, int destSize, int len){
...
template<class TSRC, class TDEST>
int Compress(const TSRC* src, TDEST* dest, int srcSize, int destSize, int len) {
...
int TestDecoder(TUint8* src, TUint8* dest, int len){
  ...
  int index = 0;
  int lastIndex = Decompress(src, dest, 8, 7, len);
...
int TestEncoder(TUint8* src, TUint8* dest, int len){
  ...
  int index = 0;
  int lastIndex = Compress(src, dest, 7, 8, len);
  cout << endl << "After: Dest Data : " << dest << endl;
