...
bool GetNextLexicographicOrder(std::string& permutation) {
  // Step#1:
...
bool GetNextLexicographicOrder(std::vector<int>& permutation) {
  // Step#1:
  // Find the largest x such that P[x]<P[x+1].
  // (If there is no such x, P is the last permutation.)
  size_t largestI = -1;
...
void LexicographicPermutation(std::vector<int>& permutation, int count = 0) {
  ...
  std::cout << std::endl;
  if (!GetNextLexicographicOrder(permutation)) {
...
int main(int argc, char* argv[]) {
  ...
  std::string inputs[] = { "ab", "bb", "hefg", "dhck", "dkhc"};
  for (auto item : inputs) {
    std::string input = item;
    if (GetNextLexicographicOrder(input)) {
    std::cout << "Input: " << item << ": Output: " << input << std::endl;
    } else {
    std::cout << "Input: " << item << ": Last String" << std::endl;
    }
  }
  return 0;
}
