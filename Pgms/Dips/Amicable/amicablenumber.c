...
struct amicable* getAmicablePairs(int startnum, int endnum) {
	...
	if(startnum >= 1 && endnum < MAX_RANGE && startnum < endnum) {
		...
		for(val1 = startnum; val1 < endnum; val1++) {
			/* get some of all the divisors of val1 */
			val2 = sumOfAllDivisiors(val1);
			/* If val2 is within the range and is bigger than val1 and
			   if the sum of all the divisors of number val2 equals val1, we have got one pair!*/
			if(val1 < val2 && val2 <= endnum && val1 == sumOfAllDivisiors(val2)) {
