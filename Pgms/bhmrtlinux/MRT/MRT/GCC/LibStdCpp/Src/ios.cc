	...
	if (__ix < numeric_limits<int>::max())
	  {
		...
		if (_M_streambuf_state & _M_exception)
		  /*Girish: __throw_ios_failure(__N("ios_base::_M_grow_words "
				      "allocation failed"));*/
		  throw("ios_base::_M_grow_words "
				      "allocation failed");
		if (__iword)
	...
	else
	  {
	    _M_streambuf_state |= badbit;
	    if (_M_streambuf_state & _M_exception)
	      //Girish: __throw_ios_failure(__N("ios_base::_M_grow_words is not valid"));
		  throw("ios_base::_M_grow_words is not valid");
	    if (__iword)
	      _M_word_zero._M_iword = 0;
	    else
	      _M_word_zero._M_pword = 0;
