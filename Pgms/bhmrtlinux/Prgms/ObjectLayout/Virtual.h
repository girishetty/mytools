...
void CheckVirtualLayout() {
	...
	//calling 2 virtual functions of DerivedA
	//Now vptr of DerivedA with in DerivedAB is at offse 0 Bytes
	//That is, its at the begining of DerivedAB
	//First 2 entries of VTABLE pointed to by this vptr holds
	//virtual functions of DerivedA, then comes ~Derived(), and
	//then 2 other virtual functions of Derived
	//Never call destructor using vptr like this.
	long* ptr = (long*)&dab;
	CallVirtualFunction("DerivedAB", ptr, 2);

	//calling 2 virtual functions of DerivedB 
	//Now vptr of DerivedB with in DerivedAB is at offse 8 Bytes
	ptr += 2;
	CallVirtualFunction("DerivedAB", ptr, 2);

	//calling 2 virtual functions of Base
	//Now vptr of Base with in DerivedAB is at offse 20 Bytes
	ptr += 3;
	CallVirtualFunction("DerivedAB", ptr, 2);
}
